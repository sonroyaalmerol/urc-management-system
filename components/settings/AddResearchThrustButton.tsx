...
interface AddResearchThrustButtonProps {
  universityMissionId: string
}

const AddResearchThrustButton: React.FC<AddResearchThrustButtonProps> = (props) => {
  ...
  const onSubmit: SubmitHandler<Partial<ResearchThrust>> = async data => {
    setSubmitting(true)
    ...
    if (res.success) {
      ...
      toast({
        title: 'Success!',
        description: 'Successfully added Research Thrust!',
        status: 'success'
      })
    } else {
      toast({
  ...
  return (
    <>
      <Button leftIcon={<AddIcon />} onClick={onOpen}>Add Research Thrust</Button>
      <Modal blockScrollOnMount={false} isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent as="form" onSubmit={handleSubmit(onSubmit)}>
          <ModalHeader>Add Research Thrust</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
          <VStack spacing={4}>
...
export default AddResearchThrustButton