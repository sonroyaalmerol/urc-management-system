...
import type { ExtendedProfile, ExtendedProject } from '../../../types/profile-card'
...
const ResearchAreaSection: React.FC<ResearchAreaSectionProps> = (props) => {
  ...
  const [researchAreas, setResearchAreas] = React.useState(profile.research_areas ?? [])
  ...
  const onSubmit: SubmitHandler<ResearchArea> = async (data) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/management/profiles/${profile.id}/research_areas`, {
      method: 'POST',
