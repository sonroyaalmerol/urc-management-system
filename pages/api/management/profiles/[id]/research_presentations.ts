...
import type { Profile, ResearchPresentation } from '@prisma/client'
...
const postHandler = async (req: NextApiRequest, res: NextApiResponse, session: Session) => {
  const body = JSON.parse(req.body) as Partial<ResearchPresentation>
  ...
  await prisma.$transaction([
    prisma.profileToResearchPresentationBridge.deleteMany({
      where: {
    ...
    prisma.researchPresentation.delete({
      where: {
        id: body.id
      }
    })
